(function($) {
	...
	$.fn.bootstrapAjaxTreeTable = function(options, param) {
		// 如果是调用方法
		if (typeof options == 'string') {
			return $.fn.bootstrapAjaxTreeTable.methods[options](this, param);
		}

		// 如果是初始化组件
		options = $.extend({}, $.fn.bootstrapAjaxTreeTable.defaults, options || {});
		// 是否有radio或checkbox
		...
		target.renderRow = function(tr,item){
			$.each(options.columns, function(index, column) {
				...
				if(index==0 && column.field=='selectItem'){
					...
					tr.append(td);
				} else if (column.field == 'lastControl') { //放操作按钮
					var td = $('<td style="text-align:center;"></td>');
					if (column.fieldVal != null) {
						td.html(column.fieldVal);
					}
					tr.append(td);
				} else {
						...
						if ($(item).parent().parent().attr('load')!='true'){
							$(item).click(function(){
								target.loadChilds($(item).parent().parent(),
									{"parentId":$(this).parent().parent().attr('data-u'),
		...
		target.repainExpends = function(){
			...
			$.each(trExpends, function(index, item) {
				...
				if ($(item).attr('isLeaf') == 'true') {
					$(item).find("span:last-child").addClass(options.leafNodeClass);
				} else {
					if ($(item).attr('load') =='true') {
						$(item).find("span:last-child").addClass(options.expanderExpandedClass);
					} else {
						$(item).find("span:last-child").addClass(options.expanderCollapsedClass);
					}
				}
		...
		target.loadChilds = function(parentTR, parms){
			...
			if (parentTR == null || parentTR.attr("isLeaf") == 'true') {
				return ;
			}
...
//                   else{
					target.renderChildRows(data, {"parentId":parms.parentId}, parms.parentIndex, _tr);
					target.treegrid({
						treeColumn: options.expandColumn?options.expandColumn:(hasSelectItem?1:0),//如果有radio或checkbox默认第二列层级显示，当前是在用户未设置的提前下
						expanderExpandedClass : options.expanderExpandedClass,
						expanderCollapsedClass : options.expanderCollapsedClass
...
//                   }
	...
	// 组件方法封装........
	$.fn.bootstrapAjaxTreeTable.methods = {
	...
	$.fn.bootstrapAjaxTreeTable.defaults = {
		id : 'menuId',// 选取记录返回的值
		...
		leafNodeClass: 'glyphicon glyphicon-file'
	};
})(jQuery);